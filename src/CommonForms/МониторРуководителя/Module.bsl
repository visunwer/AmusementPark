...
Процедура ПосещенияАттракционовВыбор(Элемент, ЗначениеДиаграммы, СтандартнаяОбработка)
	...
	Аттракцион = ЗначениеДиаграммы.Серия.Значение; //СправочникСсылка.Аттракционы
	День = ЗначениеДиаграммы.Точка.Значение; //Дата
...
Процедура СформироватьДиаграммуПосещенияАттракционов()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.ПосещенияАттракционов.Обороты(&НачалоПериода, &КонецПериода, День,) КАК
		|		ПосещенияАттракционовОбороты
		|
		|УПОРЯДОЧИТЬ ПО
		|	День";
...
// Настройки отчета детализация посещений.
// 
// Параметры:
...
//  ПользовательскиеНастройкиКомпоновкиДанных - Настройки отчета детализация посещений
